...
export async function POST(req: Request) {
  try {
    ...
    // Insert into Supabase (if table exists, otherwise it might fail silently if RLS blocks, but we handle it)
    await supabase.from("access_logs").insert({
      user_id: user.id,
      email: email,
      portal_name: portalName || "Skingif1",
    ...
    if (!isAuthorized && process.env.SMTP_USER && process.env.SMTP_PASSWORD) {
      ...
      const mailOptions = {
        ...
        subject: `🚨 ALERTA: Acceso NO autorizado en ${portalName || "Skingif1"}`,
        html: `
          <h2>Alerta de Seguridad</h2>
          <p>Un usuario no autorizado ha entrado al portal <b>${portalName || "Skingif1"}</b>.</p>
